...
export function TabOverlay({ children, active, ...props }: React.HTMLProps<HTMLDivElement> & { active: boolean }) {
  return (
    <div
      className={`flex h-full w-[200px] items-center justify-between bg-zinc-100 px-8 font-bold hover:bg-zinc-200 ${clsx({ 'border-b-4 border-blue-800': active })}`}
      {...props}
    >
      {children}
